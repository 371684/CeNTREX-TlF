...
def generate_coupled_hamiltonian_B_sqlite(QN, con, progress=False):
    with con:
        ...
        for table in [
            ...
            for i, a in tqdm(
                enumerate(QN), total=len(QN), disable=not progress, desc=desc
            ):
                for j in range(i, len(QN)):
                    b = QN[j]
                    val = (1 * a) @ (
                        eval(f"centrex.hamiltonian.hamiltonian_B_terms_coupled.{tab}")(
                            b
                        )
                    )
                    if val != 0:
...
def generate_uncoupled_hamiltonian_X_sqlite(QN, con, progress=False):
    ...
    for table in "Hff_alt, HSx, HSy, HSz, HZx, HZy, HZz".split(","):
        ...
        for a in tqdm(QN, desc=desc):
            for b in QN:
                val = (1 * a) @ (
                    eval(f"centrex.hamiltonian.hamiltonian_terms_uncoupled.{tab}")(b)
                )
                if val != 0:
                    try:
...
def create_transformation_uncoupled_to_coupled(con):
    ...
    try:
        ...
        cur.execute(
            f"""CREATE TABLE {table} (J int, mJ int, I1 real, m1 real, I2 real, m2 real, Jc int, F1 real, F int, mF int, I1c real, I2c real, value_real real, value_imag real,
...
def generate_ED_ME(QN, pol_vecs, con, progress=False):
            ...
            for table in ["ED_ME_coupled"]:
                ...
                for i, a in tqdm(
                    ...
                    for j, b in enumerate(QN):
                        val = centrex.couplings.matrix_elements.ED_ME_coupled(
                            a, b, pol_vec, rme_only=False
                        )
                        if val != 0:
                            try:
    ...
    with con:
        ...
        for table in ["ED_ME_coupled_rme"]:
            for i, a in tqdm(
                enumerate(QN), total=len(QN), desc=desc, disable=not progress
            ):
                for j, b in enumerate(QN):
                    val = centrex.couplings.matrix_elements.ED_ME_coupled(
                        a, b, rme_only=True
...
def generate_transitions(fname, QN_X, QN_B, E, B, nprocs):
    QN, H_tot = centrex.transitions.calculate_energies(QN_X, QN_B, E, B, nprocs=nprocs)
...
def generate_pre_calculated(nprocs, db_path=None):
    ...
    generate_transitions(db_path / (db + ".pickle"), QN_X, QN_B, E, B, nprocs=nprocs)
